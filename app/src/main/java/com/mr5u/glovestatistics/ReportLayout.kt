...
object ReportLayout {
     ...
     * 生成本月报表的内容。
     *
     * v0.5.0 起厂房工作下线，导出的图**只包含「家里手套计件明细」这一张表**：
     * 原来那张「各手套种类汇总」不再画，按天的汇总表也不再画，
     * 数量与金额都收在表格自己的合计行里。
     *
     * 纯计算、不画图，因此可以在不启动 Android 的情况下把「每一列够不够宽」
     * 这类问题用单元测试卡住（见 .tooling/domain-check.ps1）。
     */
    fun plan(summary: MonthSummary): Report {
        val rows = summary.days.flatMap { it.homeRecords }
        ...
        val blocks = listOf(
            ...
            Block(
                Block.Kind.FOOTER,
                lines = listOf(
                    "统计月份：${Fmt.month(summary.month)}　·　${summary.workedDays} 天　·　${summary.quantity} 双" to "",
